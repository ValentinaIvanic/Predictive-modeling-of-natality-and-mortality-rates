...
scaler = MinMaxScaler()
scaled_deaths = scaler.fit_transform(deaths.values.reshape(-1, 1))



X, y = df_to_X_y(pd.Series(scaled_deaths.flatten(), index=deaths.index), 10)
...
plt.title('Broj umrlih po godini')
plt.show()


last_window = scaled_deaths[-10:]
future_predictions = []
